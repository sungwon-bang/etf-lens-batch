...
async function login() {
  ...
  try {
    ...
    await page.locator('a[href*="MDCCOMS001.cmd"]').first().click({ timeout: 20_000 });
    await page.waitForTimeout(2_000);
    const frame = page.frames().find((item) => item.url().includes('login.jsp'));
    if (!frame) throw new Error('KRX 로그인 프레임을 찾지 못했습니다.');
    await frame.locator('input[name="mbrId"]').first().fill(id);
...
module.exports = { login, SESSION_PATH };
